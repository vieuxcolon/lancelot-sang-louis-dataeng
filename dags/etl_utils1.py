...
def load_fatalities_to_postgres():
    """
    Robust loader for the final fatalities_clean.csv into Postgres.
    Uses globals: DB_CONFIG and DATA_DIR.
    """
    ...
    try:
        ...
        cur.execute(f"""
            CREATE TABLE {DB_CONFIG['fatalities_table']} (
        ...
        execute_batch(
            cur,
            f"""
            INSERT INTO {DB_CONFIG['fatalities_table']} (date, no_of_fatalities, country)
            VALUES (%s, %s, %s)
            """,
            records,
            page_size=1000
        ...
        conn.commit()
        print(f"✔ {len(records)} rows inserted into {DB_CONFIG['fatalities_table']}")
...
def create_fatalities_clean():
    """
    Master cleaner that merges all fatalities CSVs, writes
    fatalities_clean.csv, and loads it into Postgres.
    Uses globals: DATA_DIR and DB_CONFIG.
    """
    import os
    import pandas as pd
    ...
    # Load into Postgres using robust loader
