...
def create_dimensions_and_fact():
    ...
    # ==================================================
    tables = [
        "fact_accidents", "dim_location", "country_synonym", "dim_country",
        "original_dim_location", "dim_date", "dim_employer", "dim_hazard",
        "dim_accident_type"
    ]
    for t in tables:
    ...
    df_orig_loc = pd.concat([
        df_aria[["municipality", "department", "country"]] if all(c in df_aria.columns for c in ["municipality", "department", "country"]) else pd.DataFrame(),
        df_fatal.rename(columns={"city":"municipality", "state":"department"}).loc[:, ["municipality","department","country"]] if all(c in df_fatal.columns for c in ["city","state","country"]) else pd.DataFrame(),
        df_work.rename(columns={"city":"municipality", "state":"department"}).loc[:, ["municipality","department","country"]] if all(c in df_work.columns for c in ["city","state","country"]) else pd.DataFrame(),
    ], ignore_index=True)
    ...
    # Load dim_location to memory for merging
    ...
    def build_fact(df, date_col, employer_col, hazard_col, acc_type_col):
        ...
        df2["date_id"] = df2.get("date_id", 0).fillna(0).astype(int)
        ...
        df2["location_id"] = df2.get("location_id", 0).fillna(0).astype(int)
    ...
    # Ensure source DataFrames have correct location columns
    df_aria_fact  = df_aria.rename(columns={"municipality":"municipality","department":"department"})
    df_fatal_fact = df_fatal.rename(columns={"city":"municipality","state":"department"})
    df_work_fact  = df_work.rename(columns={"city":"municipality","state":"department"})

    df_fact_list.append(build_fact(df_aria_fact, "incident_date", "employer", "hazard_class", None))
    df_fact_list.append(build_fact(df_fatal_fact, "date_of_incident", "employer", "hazard_description", "accident_type"))
    df_fact_list.append(build_fact(df_work_fact, "accident_date", "employer", "nature", "naturetitle"))
    ...
    print("✔ Fully clean star schema created (safe, deterministic, no KeyErrors)")
...
def create_dimensions_and_fact():
    ...
    tables_to_drop = [
        "fact_accidents", "dim_location", "country_synonym", "dim_country",
        "original_dim_location", "dim_date", "dim_employer", "dim_hazard", "dim_accident_type"
    ]
    ...
    df_orig_loc = pd.concat([
        df_aria[["municipality", "department", "country"]] if all(c in df_aria.columns for c in ["municipality","department","country"]) else pd.DataFrame(),
        df_fatal[["city", "state", "country"]].rename(columns={"city":"municipality","state":"department"}) if all(c in df_fatal.columns for c in ["city","state","country"]) else pd.DataFrame(),
        df_work[["city", "state", "country"]].rename(columns={"city":"municipality","state":"department"}) if all(c in df_work.columns for c in ["city","state","country"]) else pd.DataFrame(),
    ], ignore_index=True).dropna(how="all").drop_duplicates()
    ...
    # Load dim_location as DataFrame for merges
    df_dim_location = pd.read_sql("SELECT * FROM dim_location", conn)
    ...
    # ==================================================
    def build_fact(df, date_col, employer_col, hazard_col, acc_type_col,
                   df_dates, df_location, df_emp, df_haz, df_act):
        df2 = df.copy()
        ...
        df2 = df2.merge(
            df_location[["municipality","department","country","location_id"]],
    ...
    df_fact = pd.concat([
        build_fact(df_aria, "incident_date", "employer", "hazard_class", None,
                   df_dates, df_dim_location, df_emp, df_haz, df_act),
